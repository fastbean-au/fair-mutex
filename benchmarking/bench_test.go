...
// lockAdapter wraps any object that has Lock, Unlock, RLock, RUnlock methods.
type lockAdapter struct {
	v reflect.Value
}

func newLockAdapter(x any) *lockAdapter {
	return &lockAdapter{v: reflect.ValueOf(x)}
}
...
// runWithTimeout ensures the benchmark body completes within duration.
// If exceeded, the test fails immediately.
...
// --------------------------------------
// Benchmark helpers (lock focus only)
// --------------------------------------
